...
export default function Profile() {
      ...
      setReviews(rs => [
        {
          ...
          module_name: teachingByModule.find(m => m.module_code === newModule)?.module_name || '',
          can_edit: true,
          replies:  [],
          reply_count: 0, 
        },
      ...
      setReviews(rs => rs.map(x =>
        x.id === r.id
          ? { ...x,
              ...
              module_name: teachingByModule.find(m => m.module_code === updatedData.module_code)?.module_name || x.module_name,
            }
          : x
      ));
      setEditingId(null);
    ...
    setReviews(prev => prev.map(x =>
      x.id !== rid
        ...
        : {
            ...
            replies: x.replies.map(rp =>
              rp.id === repId
                ? { ...rp, is_liked: response.liked, likes_count: response.likes_count }
                : rp
            )
          }
    ));
  ...
  const toggleReplies = useCallback(async (reviewId) => {
      ...
      if (!isCurrentlyVisible && !repliesAlreadyLoaded && review && review.reply_count > 0) {
          ...
          try {
              ...
              setRepliesPagination(p => ({ ...p, [reviewId]: { loading: false } }));
          }
      }
  ...
  const handleShowMoreReplies = useCallback(async (reviewId) => {
      const pagState = repliesPagination[reviewId] || { page: 1, loading: false };
      if (pagState.loading) return;

      const nextPage = pagState.page + 1;
      setRepliesPagination(p => ({ ...p, [reviewId]: { ...pagState, loading: true } }));
      ...
      try {
          ...
          setRepliesPagination(p => ({ ...p, [reviewId]: { ...pagState, loading: false } }));
      }
  ...
  return (
    <div style={{ padding:'2rem', background:'#f0fcff', minHeight:'100vh' }}>
      <div style={{
        ...
        {prof.department && <p><b>Dept:</b> {prof.department}</p>}
            ...
            {teachingByModule.map(m => (
              <div key={m.module_code} style={{ marginBottom:'1rem' }}>
                ...
                <ul style={{ margin:'0.25rem 0 0 1.25rem', padding:0 }}>
                  {m.offerings.map((sem,i) => <li key={i}>{sem}</li>)}
                </ul>
              </div>
            ))}
          </div>
          ...
          <div>
            {reviews.map(r => {
                ...
                <div key={r.id} style={{ borderBottom:'1px solid #ddd', padding:'1rem 0' }}>
                  <Link
                    to={isLoggedIn && user?.username===r.username ? '/profile' : `/users/${encodeURIComponent(r.username)}`}
                  ...
                  {editingId === r.id ? (
                    <div>
                      <select value={editModule} onChange={e => setEditModule(e.target.value)} style={{ width:'100%', padding:'.5rem', margin:'.5rem 0' }}>
                        <option value="">Select module…</option>
                        {teachingByModule.map(m=>( <option key={m.module_code} value={m.module_code}> {m.module_code} — {m.module_name} </option> ))}
                      </select>
                      <StarRating value={editRating} onChange={setEditRating}/>
                      <textarea value={editText} onChange={e=>setEditText(e.target.value)} style={{ width:'100%', padding:'.5rem', height:'80px', margin:'.5rem 0' }} />
                    ...
                    <div>
                      ...
                      <small style={{ color:'#555' }}> {new Date(r.timestamp).toLocaleString()} </small>
                      <div style={{ marginTop:'.5rem' }}>
                        <IconBtn onClick={()=>handleLike(r)} title={r.is_liked?'Unlike':'Like'} style={{ color:r.is_liked?'red':'inherit', width:'auto', paddingRight: '8px' }}>
                          {r.is_liked?<FaHeart/>:<FaRegHeart/>}
                          <span style={{ marginLeft:'6px', fontSize:'1rem' }}> {r.likes_count} </span>
                        </IconBtn>
                        {r.can_edit && (
                          <div>
                            <IconBtn onClick={()=>startEditReview(r)} title="Edit"><FaEdit/></IconBtn>
                            <IconBtn onClick={()=>handleDelete(r)} title="Delete"><FaTrashAlt/></IconBtn>
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  <div style={{ marginTop:'.5rem' }}>
                    <button onClick={()=>setShowReplyForm(f=>({...f,[r.id]:!f[r.id]}))} style={{ display:'inline-flex', alignItems:'center', marginRight:'8px' }} >
                      {showReplyForm[r.id]? <FaTimes/> : <FaReply/>}
                      <span style={{ marginLeft:'4px' }}> {showReplyForm[r.id]? 'Cancel':'Reply'} </span>
                    </button>
                    {r.reply_count > 0 &&
                        <button onClick={()=>toggleReplies(r.id)} style={{ display:'inline-flex', alignItems:'center' }} >
                            {showReplies[r.id]? <FaChevronUp/> : <FaChevronDown/>}
                            <span style={{ marginLeft:'4px' }}>
                                {showReplies[r.id] ? `Hide Replies (${r.reply_count})` : `View Replies (${r.reply_count})`}
                            </span>
                        </button>
                    }
                  </div>
                  ...
                  {showReplyForm[r.id] && (
                    isLoggedIn ? (
                      <div id={`reply-form-${r.id}`} style={{ margin:'8px 0 0 24px' }}>
                        <textarea placeholder="Write a reply…" value={replyFormState.newText||''} onChange={e=>handleReplyText(r.id,e.target.value)} style={{ width:'100%', padding:'8px', height:'60px' }} />
                        {replyFormState.newError && ( <div style={{ color:'red', marginTop:'4px' }}> {replyFormState.newError} </div> )}
                        <IconBtn onClick={()=>submitReply(r.id)} title="Post reply"><FaPaperPlane/></IconBtn>
                      </div>
                    ) : ( <p style={{ marginLeft:'1rem' }}><Link to="/login">Log in</Link> to reply.</p> )
                  )}

                  {showReplies[r.id] && (
                    <div style={{ margin:'12px 0 0 24px' }}>
                      {replyPagState.loading && r.replies.length === 0 ? (<p>Loading replies...</p>) : (
                       <div>
                          {r.replies.map(rep => (
                              replyFormState.editingReplyId === rep.id ? (
                              <div key={rep.id} style={{ marginBottom:'8px' }}>
                                  <textarea value={replyFormState.editText||''} onChange={e=>setReplyState(s=>({ ...s, [r.id]:{ ...s[r.id], editText:e.target.value } }))} style={{ width:'100%', padding:'8px', height:'60px' }} />
                                  {replyFormState.editError && ( <div style={{ color:'red', marginTop:'4px' }}> {replyFormState.editError} </div> )}
                                  <IconBtn onClick={()=>handleReplyEdit(r.id,rep)} title="Save"><FaCheck/></IconBtn>
                                  <IconBtn onClick={()=>setReplyState(s=>({ ...s, [r.id]:{ ...s[r.id], editingReplyId:null, editText:'', editError:'' } }))} title="Cancel"><FaTimes/></IconBtn>
                              </div>
                              ) : (
                              <div key={rep.id} style={{ borderLeft:'2px solid #eee', paddingLeft:'12px', marginBottom: '12px' }}>
                                  <Link to={isLoggedIn && user?.username===rep.username ? '/profile' : `/users/${encodeURIComponent(rep.username)}`} style={{ color:'#0077cc', fontWeight:'bold', textDecoration:'none' }} >
                                    {rep.username}
                                  </Link>
                                  <p style={{ margin:'4px 0' }}>{parseMentions(rep.text)}</p>
                                  <small style={{ color:'#555' }}> {new Date(rep.timestamp).toLocaleString()} </small>
                                  <div style={{ marginTop:'4px' }}>
                                  <IconBtn onClick={()=>handleReplyLike(r.id,rep.id)} title={rep.is_liked?'Unlike':'Like'} style={{ color:rep.is_liked?'red':'inherit', width:'auto', paddingRight: '8px' }} >
                                      {rep.is_liked?<FaHeart/>:<FaRegHeart/>}
                                      <span style={{ marginLeft:'6px', fontSize:'1rem' }}> {rep.likes_count} </span>
                                  </IconBtn>
                                  <IconBtn onClick={()=>startNestedReply(r.id,rep)}><FaReply/></IconBtn>
                                  {rep.can_edit && (
                                      <div>
                                      <IconBtn onClick={()=>startReplyEdit(r.id,rep)} title="Edit"><FaEdit/></IconBtn>
                                      <IconBtn onClick={()=>handleReplyDelete(r.id,rep.id)} title="Delete"><FaTrashAlt/></IconBtn>
                                      </div>
                                  )}
                                  </div>
                              </div>
                          ...
                          {replyPagState.hasMore && (
                              <button onClick={() => handleShowMoreReplies(r.id)} disabled={replyPagState.loading}>
                                  {replyPagState.loading ? 'Loading...' : 'Show More Replies'}
                              </button>
                          )}
                       </div>
                      )}
