    ...
    async function loadMyReviews() {
      try {
        ...
        const withNames = await Promise.all(
          rv.map(async r => {
            const res = await fetch(`${API_URL}/professor/${r.prof_id}`, {
        ...
        <div style={{ marginBottom:'1rem' }}>
          {editingUsername ? (
            <>
              ...
              <button onClick={() => setEditingUsername(false)} style={{ marginLeft:'.5rem' }}>
                Cancel
              </button>
            </>
          ) : (
        ...
        <div style={{ marginBottom:'1rem' }}>
          {editingPassword ? (
            <>
              ...
              <button onClick={() => setEditingPassword(false)} style={{ marginLeft:'.5rem' }}>
                Cancel
              </button>
            </>
          ) : (
            ...
            <div key={r.id} style={{
              ...
              {editingId === r.id ? (
                <>
                  ...
                  <button onClick={() => handleEdit(r)} style={{ marginRight:'.5rem' }}>
                    Save
                  </button>
                ...
                <>
                  <Link to={`/professor/${r.prof_id}`} style={{ color:'#0077cc', textDecoration: 'none', fontWeight: 'bold' }}>
                    {r.prof_name || `Professor ${r.prof_id}`}
                  </Link>
                  <p><b>Module:</b> {r.module_code} — {r.module_name}</p>
                  <p><b>Rating:</b> {r.rating}</p>
                  <p>{r.text}</p>
                  <button onClick={() => startEdit(r)} style={{ marginRight:'.5rem' }}>
                    Edit
                  </button>
