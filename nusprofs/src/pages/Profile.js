...
export default function Profile() {
  ...
  const reload = useCallback(async()=>{
    ...
    try {
      ...
      const sum = await fetch(`${API_URL}/professor/${id}/review_summary`,{headers:{Accept:'application/json'}});
      if(!sum.ok) throw new Error(sum.statusText);
      const { average_rating, review_count } = await sum.json();
  ...
  const handleLike = async r=>{
    ...
    setReviews(rs=>rs.map(x=>
      x.id===r.id
        ?{...x,is_liked:!x.is_liked,likes_count:x.is_liked?x.likes_count-1:x.likes_count+1}
        :x
    ));
  };
  ...
  const submitReply = async rid=>{
    ...
    if(!rs.newText){
      return setReplyState(s=>({...s,[rid]:{...s[rid],newError:'Reply cannot be empty'}}));
    }
    try {
      await createReply(rid,rs.newText,rs.replyToId);
      const updated = await getReplies(rid);
      setReviews(prev=>prev.map(x=>x.id===rid?{...x,replies:updated}:x));
      setReplyState(s=>({...s,[rid]:{...s[rid],newText:'',replyToId:null,newError:''}}));
      setShowReplyForm(f=>({...f,[rid]:false}));
      setShowReplies(f=>({...f,[rid]:true}));
    } catch(e){
      setReplyState(s=>({...s,[rid]:{...s[rid],newError:e.message}}));
    }
  };

  const startReplyEdit = (rid,rep)=>{
    initReply(rid);
    setReplyState(s=>({...s,[rid]:{
      ...s[rid],
  ...
  const handleReplyEdit = async (rid,rep)=>{
    ...
    const rs=replyState[rid]||{};
    if(!rs.editText){
      return setReplyState(s=>({...s,[rid]:{...s[rid],editError:'Text required'}}));
    }
    try {
      ...
      setReplyState(s=>({...s,[rid]:{...s[rid],editError:e.message}}));
    }
  };
    ...
    setReviews(prev=>prev.map(x=>{
      ...
      return {
        ...
        replies: x.replies.map(rp=>
          rp.id===repId
            ?{...rp,is_liked:!rp.is_liked,likes_count:rp.is_liked?rp.likes_count-1:rp.likes_count+1}
            :rp
        )
      };
    }));
  ...
  const toggleReplies    = rid => setShowReplies(s=>({...s,[rid]:!s[rid]}));
                ...
                {r.replies.slice().reverse().map(rep=>(
                  replyState[r.id]?.editingReplyId===rep.id ? (
                    <div key={rep.id} style={{marginBottom:'8px'}}>
                      ...
                      <IconBtn onClick={()=>setReplyState(s=>({...s,[r.id]:{
                        ...s[r.id],
                        editingReplyId: null,
                        editText:       '',
                        editError:      ''
